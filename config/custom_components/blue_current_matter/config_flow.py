...
from homeassistant.const import CONF_API_TOKEN, CONF_IGNORE
...
class BlueCurrentConfigFlow(ConfigFlow, domain=DOMAIN):
    ...
    async def async_step_user(
        ...
        if user_input is not None:
            if user_input[CONF_IGNORE]:  # Ignore API token setting
                return self.async_create_entry(title="Ignored", data=user_input)
